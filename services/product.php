...
<?php
// Include the PDO connection setup file (assuming it's named config.php)


// Check to make sure the id parameter is specified in the URL
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    // Prepare statement and execute, prevents SQL injection
    $stmt = $pdo->prepare('SELECT * FROM category WHERE (title LIKE "%garden%" OR title LIKE "%plant%") AND id = :id');
