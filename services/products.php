...
$stmt = $pdo->prepare('SELECT * FROM category where `title`= "garden" OR `title`= "gardening" OR`title`= "plantcare" ORDER BY date_added DESC LIMIT ?,?');
...
$total_products = $pdo->query('SELECT * FROM category where `title`= "garden" OR `title`= "gardening" OR`title`= "plantcare" ')->rowCount();
?>
