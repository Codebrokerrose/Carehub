...
$stmt = $pdo->prepare('SELECT * FROM category where `title` LIKE "%garden%" OR `title` LIKE "%plant%" ORDER BY date_added DESC LIMIT ?,?');
...
$total_products = $pdo->query('SELECT * FROM category where `title` LIKE "%garden%" OR `title` LIKE "%plant%" ')->rowCount();
?>
